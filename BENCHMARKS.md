...
An optional local transformer NER sidecar can be configured through `PII_SECONDARY_CLASSIFIER_URL` and is merged with GLiNER before exact-span resolution. Its independent Nemotron run reached 44.85% person-name precision, 27.80% recall, and 34.33% F1. It improves recall relative to heuristic-only detection but is not sufficient for enforcement and has not been promoted.
...
For v1.12.0, `pipeline/benchmarks/generate_guard_hard_negatives.py` generated 60 benign trigger-word training records and `train_guard_model.py` was extended with `--extra-jsonl` and `--extra-calibration-jsonl` so those records can be used without changing the deployable model schema. This did not fix NotInject for the compact guard: the calibrated hard-negative artifact still produced 209 true negatives, 130 false positives, 61.65% accuracy, and 38.35% false-positive rate on the pinned 339-case NotInject split. The extra data path is useful infrastructure, but the measured result says the compact linear model still needs a different representation or a stronger guard model for over-defense.
...
python pipeline/benchmarks/generate_guard_hard_negatives.py --output .benchmark-data/benign-trigger-hard-negatives.jsonl --matrix-output .benchmark-data/benign-trigger-hard-negatives-matrix.json --special-tokens-output .benchmark-data/benign-trigger-special-tokens.json
python pipeline/benchmarks/train_guard_model.py --output .benchmark-data/guard-hardnegative-calibrated.json --extra-jsonl .benchmark-data/benign-trigger-hard-negatives.jsonl --extra-calibration-jsonl .benchmark-data/benign-trigger-hard-negatives.jsonl
python pipeline/benchmarks/run_injection_matrix.py --matrix .benchmark-data/notinject-matrix.json --model .benchmark-data/guard-hardnegative-calibrated.json --output gateway/benchmarks/results/v1.12.0-notinject-hardnegative-calibrated-guard.json --repetitions 1 --minimum 0 --maximum-fpr 1
...
For v1.12.0, `pipeline/benchmarks/analyze_emscad_false_negatives.py` was added to reproduce the deterministic TF-IDF split and inspect held-out fraud misses by EMSCAD structured fields. On the current local full CSV split it measured 3,576 test records, 149 true positives, 3,386 true negatives, 17 false positives, 24 false negatives, 98.85% accuracy, 89.76% precision, 86.13% recall, 87.91% F1, and 0.50% false-positive rate. The largest false-negative slices were missing `required_education` (15/24 false negatives), missing `required_experience` (12/24), full-time jobs (11/24), missing industry/function (7/24), and Information Technology / Information Technology and Services records (6/24 each, 75% miss rate within their held-out fraud subsets). Six false negatives scored between 0.5 and 0.6 and three scored between 0.6 and 0.7, so some misses are near-threshold; the lower-scoring misses still need new signal.
...
`scripts/scrape_and_enrich_jobs.py` also supports a bounded Apify/WhoisJSON collection pass. The July 2026 200-listing LinkedIn actor sample did not contain company websites or contact emails, so no domain or Whois feature was inferred. The ignored raw output is labeled `unknown` and is not training or release evidence.
...
python benchmarks/analyze_emscad_false_negatives.py --input ../.benchmark-data/emscad.csv --output-json ../gateway/benchmarks/results/v1.12.0-emscad-false-negative-analysis.json --output-markdown ../gateway/benchmarks/results/v1.12.0-emscad-false-negative-analysis.md --false-negatives-csv ../gateway/benchmarks/results/v1.12.0-emscad-false-negatives.csv
python benchmarks/evaluate_emscad_ensemble.py --input ../.benchmark-data/emscad.csv --transformer-model ../.benchmark-data/emscad-deberta-weighted-v112 --output ../gateway/benchmarks/results/v1.12.0-emscad-ensemble.json --max-fpr 0.003526300323244196
