...
export class JwtError extends Error {
    public constructor(message: string, public readonly code: 'malformed' | 'unsupported_alg' | 'bad_signature' | 'expired' | 'not_yet_valid' | 'issuer_mismatch' | 'audience_mismatch' | 'missing_tenant' | 'invalid_claims') {
        super(message);
        this.name = 'JwtError';
    }
}
...
function verifySignature(signingInput: string, signatureB64: string, header: JwtHeader, options: VerifyJwtOptions): void {
...
export function createJwtAuth(config: JwtAuthConfig, eventSink?: JwtAuthEventSink): RequestHandler {
    ...
    return function jwtAuth(req: AuthenticatedRequest, res: Response, next: NextFunction): void {
        ...
        try {
            ...
            Object.assign(options, { algorithm }, config.secret === undefined ? {} : { secret: config.secret }, config.publicKeyPem === undefined ? {} : { publicKeyPem: config.publicKeyPem }, config.issuer === undefined ? {} : { issuer: config.issuer }, config.audience === undefined ? {} : { audience: config.audience }, config.clockSkewSeconds === undefined
                ? {}
