/**
 * Valence Gateway - Prompt Injection Shield
 *
 * Inbound interceptor scoring prompts for systemic injection intent:
 * instruction-override phrasing, persona-jailbreak framing, control-token
 * smuggling, system-prompt exfiltration asks, and obfuscation carriers.
 *
 * Same pluggable shape as the PII engine: `InjectionDetector` is the
 * contract, `HeuristicInjectionDetector` is the static-rule tier, and
 * `GuardModelDetector` adapts a cognitive micro-model (Llama-Guard-class)
 * behind an injected `GuardModelClient` transport.
 *
 * Scoring model: each rule carries a weight; the shield sums the weights
 * of distinct matched rules per detector, takes the maximum detector
 * score, and blocks when it meets the threshold. Distinct-rule summation
 * means one repeated phrase cannot be diluted or inflated by repetition,
 * while genuinely layered attacks (override + exfiltration + smuggling)
 * accumulate quickly.
 *
 * Failure semantics mirror the PII scanner: any detector rejection throws
 * `InjectionShieldError`; the pipeline maps that to the active
 * SECURITY_MODE. Under FAIL_CLOSED a broken detector blocks traffic.
 */

/** One matched rule, with a short excerpt safe for structured logs. */
export interface InjectionMatch {
  readonly ruleId: string;
  readonly description: string;
  readonly weight: number;
  /** Truncated to `EXCERPT_LIMIT` chars - never log full payloads. */
...
export interface InjectionVerdict {
  ...
  /** Highest per-detector cumulative weight observed. */
...
export class InjectionShieldError extends Error {
  ...
  public constructor(failedDetectors: readonly string[], cause?: unknown) {
    super(
      `Injection analysis incomplete - detector failure(s): ${failedDetectors.join(', ')}`,
    );
...
/**
 * Canonicalization applied before rule matching, defeating the cheap
 * obfuscations first: Unicode compatibility folding (fullwidth letters,
 * ligatures), zero-width character stripping, case folding, and
 * whitespace collapse. Rules therefore match on one canonical form
 * instead of chasing variants.
 */
...
interface InjectionRule {
  readonly id: string;
  readonly description: string;
  /** Matched against the NORMALIZED (lowercased, collapsed) prompt. */
...
const INJECTION_RULES: readonly InjectionRule[] = [
  {
    ...
    pattern:
      /\b(?:ignore|disregard|forget|override|bypass)\b[^.!?]{0,40}\b(?:previous|prior|above|earlier|all|system)\b[^.!?]{0,20}\b(?:instructions?|rules?|prompts?|guidelines?|directives?)\b/,
    weight: 0.8,
  },
  {
    ...
    pattern:
      /\b(?:new|real|actual|true)\s+(?:instructions?|rules?|system prompt)\b[^.!?]{0,40}\b(?:supersede|replace|override|take precedence)\b/,
    weight: 0.7,
  },
  {
    ...
    pattern:
      /\byou are (?:now|no longer)\b[^.!?]{0,60}\b(?:restrictions?|limitations?|filters?|guidelines?|dan|developer mode|jailbroken)\b/,
    weight: 0.7,
  },
  {
    id: 'do-anything-now',
  ...
  },
  {
    ...
    pattern:
      /\b(?:pretend|act as if|imagine)\b[^.!?]{0,40}\b(?:no|without|free of)\b[^.!?]{0,20}\b(?:rules?|restrictions?|filters?|guidelines?)\b/,
    weight: 0.6,
  },
  {
    ...
    pattern:
      /\b(?:reveal|print|show|repeat|output|display|tell me)\b[^.!?]{0,40}\b(?:system prompt|initial instructions?|hidden instructions?|original instructions?|your instructions?)\b/,
    weight: 0.8,
  },
  {
    ...
    pattern:
      /<\|(?:im_start|im_end|system|endoftext|assistant)\|>|\[\/?(?:inst|system)\]|<<sys>>/,
    weight: 0.9,
  },
  {
  ...
  },
  {
    ...
    pattern:
      /\b(?:tool|function)[ _-]?(?:result|output|response)\b[^.!?]{0,30}\b(?:says?|returned|instructs?)\b[^.!?]{0,40}\b(?:you must|you should|ignore|execute)\b/,
    weight: 0.6,
  },
];
...
/* -------------------------------------------------------------------------
 * Cognitive micro-model tier
 * ---------------------------------------------------------------------- */

/** Micro-model verdict for the whole prompt. */
export interface GuardModelAssessment {
  /** e.g. "prompt_injection", "jailbreak", "benign". */
  readonly label: string;
  readonly score: number;
}

/** Transport contract to a Llama-Guard-class safety classifier. */
...
export class GuardModelDetector implements InjectionDetector {
  ...
  ): Promise<readonly InjectionMatch[]> {
    ...
    return [
      {
        ...
        // Model score maps directly onto the shield's weight scale so a
        // confident model verdict can block on its own.
...
/** Inert client for deployments without a provisioned guard model. */
...
export interface InjectionShieldOptions {
  /** Cumulative weight at which the prompt is blocked. Default 0.8. */
...
export class InjectionShield {
  ...
  public constructor(
    detectors: readonly InjectionDetector[],
    options: InjectionShieldOptions = {},
  ) {
  ...
  /**
   * Normalizes the prompt once, fans out to every detector, and renders a
   * verdict. Detector failures throw; partial verdicts are never returned.
   */
  public async evaluate(rawText: string): Promise<InjectionVerdict> {
    ...
    const settled = await Promise.allSettled(
      this.detectors.map((detector) => detector.detect(normalized)),
    );
