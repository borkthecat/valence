...
export function createGatewayMetrics(): GatewayMetrics {
  ...
  return {
    ...
    requestsTotal: registry.counter(
      'valence_requests_total',
      'Total proxied requests by outcome.',
    ),
    piiRedactionsTotal: registry.counter(
      'valence_pii_redactions_total',
      'Total PII/secret surrogates injected before upstream forwarding.',
    ),
    injectionsBlockedTotal: registry.counter(
      'valence_injections_blocked_total',
      'Total prompts blocked by the injection shield.',
    ),
    failOpenBypassTotal: registry.counter(
      'valence_fail_open_bypass_total',
      'Total subsystem failures bypassed while in FAIL_OPEN mode.',
    ),
    clientDisconnectsTotal: registry.counter(
      'valence_client_disconnects_total',
      'Total client disconnects that aborted an upstream task.',
    ),
    rateLimitedTotal: registry.counter(
      'valence_rate_limited_total',
      'Total requests rejected by the per-tenant rate limiter.',
    ),
    upstreamForwardLatencyMs: registry.histogram(
      'valence_upstream_forward_latency_ms',
      'Latency in milliseconds from request receipt to upstream forward.',
    ),
  };
}
