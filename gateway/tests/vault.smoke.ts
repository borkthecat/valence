/**
 * Token vault: tokenization, dedup stability, detokenization, restoration,
 * cross-category isolation, and revocation.
 */
...
import {
  ...
  SurrogateCategory,
  VAULT_ENTRY_TTL_MS,
} from '../src/core/crypto/tokenVault';
...
function run(): void {
  ...
  const restored = vault.restoreText(
    `Contact ${first} now, ignore [M_EMAIL_deadbeefdeadbeef]`,
  );
  assert.equal(
    restored,
    'Contact alice@example.com now, ignore [M_EMAIL_deadbeefdeadbeef]',
    'unknown surrogates left verbatim',
  );
