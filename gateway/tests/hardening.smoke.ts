/**
 * Audit-hardening regression: ReDoS bounds, per-request restoration scope,
 * and the fail-closed error boundary.
 */
...
import {
  ...
  InjectionShield,
} from '../src/core/filters/injectionShield';
import {
...
async function run(): Promise<void> {
  ...
  const payloads: Array<[string, string]> = [
    ['plain', 'a'.repeat(200_000)],
    ['email-bait', 'a.b_c%d+e-'.repeat(20_000)],
    ['at-flood', ('a'.repeat(50) + '@').repeat(3_500)],
    ['key-flood', ('sk-' + 'x'.repeat(19) + ' ').repeat(8_000)],
    ['marker-flood', '[M_'.repeat(60_000)],
    ['begin-key', '-----BEGIN PRIVATE KEY-----' + 'A'.repeat(190_000)],
  ];
  ...
  assert.throws(
    () => strict.push(`foreign ${tenantA}`),
    UnresolvedSurrogateError,
    'out-of-scope surrogate throws under fail-closed',
  );
  ...
  handler(
    new PiiScanError(['heuristic-static']),
    { method: 'POST', path: '/v1/messages' } as unknown as Request,
    preRes.res,
    () => undefined,
  );
  ...
  handler(
    new UnresolvedSurrogateError('[M_EMAIL_deadbeefdeadbeef]'),
    { method: 'POST', path: '/v1/messages' } as unknown as Request,
    midRes.res,
    () => undefined,
  );
...
function fakeResponse(headersSent: boolean): {
  res: Response;
  state: { status: number; destroyed: boolean; socketDestroyed: boolean };
} {
