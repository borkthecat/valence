...
def test_backup_restore_and_lifecycle(tmp_path):
 ...
 r=ReviewStore(tmp_path/'r.sqlite');x=r.create(CreateReview(tenant_id='t',case_id='c',candidate_id='x',source_request_id='s',policy_version='p',model_version='m',model_digest='d',evidence_snapshot_digest='e',risk='low'),'k');r.transition('t',x['review_id'],'u','claim',1);assert run_review_lifecycle(r,'t',datetime.now(UTC))['stale_claims_released']==1
