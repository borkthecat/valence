...
def test_submissions_and_frozen_cases_are_immutable(tmp_path):
 s=StudyStore(tmp_path/'a.db');s.add_case('t','c',{'job':{},'candidates':[]},['r1','r2']);s.save('t','c','r1',{'hard_eligibility':'pass'},True)
 try:s.save('t','c','r1',{'hard_eligibility':'fail'},True)
 except ValueError:pass
 else:raise AssertionError('submitted labels must be immutable')
 s.freeze('t','c')
 try:s.save('t','c','r2',{'hard_eligibility':'pass'},False)
 except ValueError:pass
 else:raise AssertionError('frozen cases must reject changes')
